...
void ObjPredictor::init() {
  /* get param */
  node_->declare_parameter("prediction.obj_num", 5);
  node_->declare_parameter("prediction.lambda", 1.0);
  node_->declare_parameter("prediction.predict_rate", 1.0);
  
  obj_num_ = node_->get_parameter("prediction.obj_num").as_int();
  lambda_ = node_->get_parameter("prediction.lambda").as_double();
  predict_rate_ = node_->get_parameter("prediction.predict_rate").as_double();
  ...
  for (int i = 0; i < obj_num_; i++) {
    ...
    auto pose_sub = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
        "/dynamic/pose_" + std::to_string(i), 10, 
        [obj_his](const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
  ...
  predict_timer_ = node_->create_wall_timer(
      std::chrono::milliseconds(static_cast<int>(1000 / predict_rate_)),
