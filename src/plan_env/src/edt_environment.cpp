...
void EDTEnvironment::setMap(std::shared_ptr<SdfNode> map) {
  this->sdf_map_ = map;
...
double EDTEnvironment::distToBox(int idx, const Eigen::Vector3d& pos, const double& time) {
  ...
  for (int i = 0; i < 3; i++) {
    dist(i) = pos(i) >= box_min(i) && pos(i) <= box_max(i) ? 0.0 : std::min(fabs(pos(i) - box_min(i)),
...
void EDTEnvironment::interpolateTrilinear(double values[2][2][2],
  ...
  grad[0] += diff[2] * (1 - diff[1]) * (values[1][0][1] - values[0][1][1]);
