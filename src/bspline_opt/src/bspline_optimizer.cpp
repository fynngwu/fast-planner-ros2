...
void BsplineOptimizer::setParam(const rclcpp::Node::SharedPtr& node) {
  logger_ = node->get_logger();

  // Helper lambda to declare or get parameter (avoid duplicate declaration)
  auto get_or_declare = [&node](const std::string& name, double default_value) -> double {
  ...
  auto get_or_declare_int = [&node](const std::string& name, int default_value) -> int {
    if (node->has_parameter(name)) {
  ...
  lambda1_ = get_or_declare("optimization.lambda1", -1.0);
  lambda2_ = get_or_declare("optimization.lambda2", -1.0);
  lambda3_ = get_or_declare("optimization.lambda3", -1.0);
  lambda4_ = get_or_declare("optimization.lambda4", -1.0);
  lambda5_ = get_or_declare("optimization.lambda5", -1.0);
  lambda6_ = get_or_declare("optimization.lambda6", -1.0);
  lambda7_ = get_or_declare("optimization.lambda7", -1.0);
  lambda8_ = get_or_declare("optimization.lambda8", -1.0);

  dist0_     = get_or_declare("optimization.dist0", -1.0);
  max_vel_   = get_or_declare("optimization.max_vel", -1.0);
  max_acc_   = get_or_declare("optimization.max_acc", -1.0);
  visib_min_ = get_or_declare("optimization.visib_min", -1.0);
  dlmin_     = get_or_declare("optimization.dlmin", -1.0);
  wnl_       = get_or_declare("optimization.wnl", -1.0);

  max_iteration_num_[0] = get_or_declare_int("optimization.max_iteration_num1", -1);
  max_iteration_num_[1] = get_or_declare_int("optimization.max_iteration_num2", -1);
  max_iteration_num_[2] = get_or_declare_int("optimization.max_iteration_num3", -1);
  max_iteration_num_[3] = get_or_declare_int("optimization.max_iteration_num4", -1);

  max_iteration_time_[0] = get_or_declare("optimization.max_iteration_time1", -1.0);
  max_iteration_time_[1] = get_or_declare("optimization.max_iteration_time2", -1.0);
  max_iteration_time_[2] = get_or_declare("optimization.max_iteration_time3", -1.0);
  max_iteration_time_[3] = get_or_declare("optimization.max_iteration_time4", -1.0);

  algorithm1_ = get_or_declare_int("optimization.algorithm1", -1);
  algorithm2_ = get_or_declare_int("optimization.algorithm2", -1);
  order_      = get_or_declare_int("optimization.order", -1);
}
...
void BsplineOptimizer::setCostFunction(const int& cost_code) {
  ...
  RCLCPP_INFO(logger_, "cost func: %s", cost_str.c_str());
}
...
Eigen::MatrixXd BsplineOptimizer::BsplineOptimizeTraj(const Eigen::MatrixXd& points, const double& ts,
  ...
  // Clear guide path if not using GUIDE cost to prevent accumulation
  if (!(cost_function & GUIDE)) {
    guide_pts_.clear();
  }
  
  // Clear waypoints if not using WAYPOINTS cost
...
void BsplineOptimizer::optimize() {
  ...
  try {
    ...
    RCLCPP_WARN(logger_, "[Optimization]: nlopt exception");
    cout << e.what() << endl;
  }
  ...
  if (!(cost_function_ & GUIDE)) {
    RCLCPP_INFO(logger_, "iter num: %d", iter_num_);
  }
}
